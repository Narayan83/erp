    ...
    try {
      ...
      setItems(
        (res.data.data || []).map((it) => ({
          ...it,
          ID: it.ID ?? it.id ?? null,
          CompanyName: it.CompanyName || it.Name || it.name || "",
  ...
  const handleSave = async () => {
    try {
      for (const it of items) {
        const companyId = it.ID ?? it.id;
        if (it._deleted && companyId) await axios.delete(`${BASE_URL}/api/companies/${companyId}`);
        else if (companyId) {
          if (it._edited) await axios.put(`${BASE_URL}/api/companies/${companyId}`, { name: it.CompanyName, code: it.Code });
        } else {
...
function EditCompanyForm({ company, onSave, onCancel }) {
  ...
  const submit = async () => {
    ...
    await onSave(companyId, code.trim(), name.trim(), setSaving);
  };
