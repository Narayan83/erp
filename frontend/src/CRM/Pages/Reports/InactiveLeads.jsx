...
const InactiveLeads = () => {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      const [leadsResp, emps, interResp] = await Promise.all([
      ...
      // Filter only inactive leads (stage = 'inactive')
      const inactiveLeads = mergedLeads.filter(lead => getStage(lead) === 'inactive');
      ...
      const allInteractions = Array.isArray(interResp) ? interResp : (interResp && interResp.data ? interResp.data : []);
      setInteractions(allInteractions);
    } catch (err) {
      console.error('Error loading inactive leads:', err);
      alert('Failed to load inactive leads. Please try again.');
    } finally {
  ...
  const getLastInteraction = (lead) => {
    ...
    const leadInteractions = interactions.filter(inter => {
      const interLeadId = inter.lead_id || inter.leadId || (inter.lead && (inter.lead.id || inter.lead.ID));
      return String(interLeadId) === String(leadId);
    });

    if (leadInteractions.length === 0) return null;

    // Sort by timestamp descending
    const sorted = leadInteractions.sort((a, b) => {
      const tsA = new Date(a.timestamp || a.Timestamp || a.created_at || a.createdAt || 0);
      const tsB = new Date(b.timestamp || b.Timestamp || b.created_at || b.createdAt || 0);
      return tsB - tsA;
    });
  ...
  const getDaysSinceLastInteraction = (lead) => {
    const lastInter = getLastInteraction(lead);
    if (!lastInter) return 'Never';
    
    const ts = lastInter.timestamp || lastInter.Timestamp || lastInter.created_at || lastInter.createdAt;
    if (!ts) return 'Never';
    
    const lastDate = new Date(ts);
    if (isNaN(lastDate)) return 'Never';
  ...
  const matchesSearch = (lead) => {
    ...
    return business.includes(term) || name.includes(term) || mobile.includes(term) || 
           email.includes(term) || empName.includes(term);
  };
  ...
  const exportToExcel = () => {
    ...
    try {
      const data = visible.map(lead => {
        ...
        const lastInter = getLastInteraction(lead);
        const lastInterDate = lastInter ? formatDateShort(lastInter.timestamp || lastInter.Timestamp || lastInter.created_at || lastInter.createdAt) : 'Never';
        ...
        return {
          ...
          'State': lead.state || '',
          'Assigned To': empName,
          'Last Interaction': lastInterDate,
          'Days Since Last Contact': getDaysSinceLastInteraction(lead),
      ...
      const fileName = `inactive_leads_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  return (
    <div className="sales-interactions-container">
      ...
      {loading ? (
        <div style={{ padding: '40px', textAlign: 'center', fontSize: '16px', color: '#666' }}>Loading inactive leads...</div>
      ) : (
        <>
          <div style={{ padding: '12px 0', fontSize: '14px', color: '#666' }}>
            Showing {paged.length} of {visible.length} inactive leads
          </div>
            ...
            <table className="si-table">
              ...
              <tbody>
                {paged.map((lead) => {
                  ...
                  const lastInter = getLastInteraction(lead);
                  const lastInterDate = lastInter ? formatDateShort(lastInter.timestamp || lastInter.Timestamp || lastInter.created_at || lastInter.createdAt) : 'Never';
                  const daysSince = getDaysSinceLastInteraction(lead);
