...
const RejectedLeads = () => {
  ...
  // Load rejected leads from backend
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `rejected_leads_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (err) {
        ...
        <>
          <div style={{ padding: '12px 0', fontSize: '14px', color: '#666' }}>
            {total === 0
              ...
              : `Showing ${startItem}-${endItem} of ${total} rejected leads`}
          </div>
